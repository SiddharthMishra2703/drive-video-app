...
export default async function handler(req, res) {
    try {
        ...
        // 3a. Filter new movies
        const newMovies = driveData.filter(m => !cached[m.id]);

        // 3b. Detect deleted movies
        const driveIds = new Set(driveData.map(m => m.id));
        const deleted = Object.keys(cached).filter(id => !driveIds.has(id));

        // Remove deleted movies from Firestore
        if (deleted.length > 0) {
            ...
            await batch.commit();
            console.log(`Deleted ${deleted.length} movies from Firestore`);
        }
      ...
      `;
            ...
            for (const movie of cleaned) {
                ...
                console.log(`Saved ${cleaned.length} movie to Firestore`);
            }
        }


        // 7. Return all movies (cached + new)
        const finalSnap = (cleaned.length || deleted.length) 
